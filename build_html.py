...
// Certified marathon distance (chip-timed). GPX is ~0.22 mi longer due to GPS drift.
const CERT_MI = 26.2188;
const CERT_KM = 42.195;
GOALS.forEach(g=>{
  g.sec_mi = g.sec_km * 1.609344;
  g.total = Math.round(g.sec_km * CERT_KM);
});
let activeGoal = "B";
...
function renderSplits(){
  ...
  // Truncate to certified marathon distance for splits + total
  const certTotal = unit==="mi" ? CERT_MI : CERT_KM;
  const fracs = ms.map(m=>{
    const startD = m.n - 1;          // start of this unit
    const endD   = Math.min(m.n, certTotal);
    return Math.max(0, endD - startD);
  });
  const adjusted = ms.map((m,i)=>{
    ...
    if(fracs[i] < 1) split = split * fracs[i];
    return split;
  });
  // normalize so total == goal.total (certified marathon time)
  const validSum = adjusted.filter(s=>s!==null).reduce((a,b)=>a+b,0);
  const factor = goal.total/validSum;
  const finalSplits = adjusted.map(s=> s===null ? null : s*factor);
  ...
  finalSplits.forEach((s,i)=>{
    ...
    const isPartial = f < 1;
    const pace = s/f;
    const certTotalLocal = unit==="mi" ? CERT_MI : CERT_KM;
    const label = isPartial ? `${(m.n-1)}-${certTotalLocal.toFixed(2)}` : m.n;
...
function renderTotalDist(){
  document.getElementById("totaldist").textContent =
    unit==="mi" ? `${CERT_MI.toFixed(2)} mi (certified)` : `${CERT_KM.toFixed(3)} km (certified)`;
}
